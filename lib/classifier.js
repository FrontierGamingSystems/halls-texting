...
const extraAliases = {
  'house-bingo-lax': ['House 8ingo','House Bingo'],
  'bd-bingo-industry': ['Industry BNGO','Industry Bingo'],
  'bd-bingo-suisun-city': ['Suisun Bingo'],
  ...
  'area-24th-street-bingo': ['24th Street Bingo', '24th St Bingo', '24th Street B'],
...
export function classify(message, directory) {
  ...
  const urlPath = u => decodeURI(u.pathname).replace(/\/+$/,'') || '/';
  for (const link of links) {
    ...
    for (const h of sameHost) {
      ...
      const sharedService = /(^|\.)(facebook\.com|yelp\.com|elks\.org|google\.com|multiplayerbingo\.com)$/.test(host(link));
      if ((sameHost.length === 1 && !sharedService) || (base !== '/' && (urlPath(link) === base || urlPath(link).startsWith(base + '/')))) add(h.id, 'known_link', link.href);
    }
  }
